...
// Example habit suggestions for rotating placeholders
...
export default function HabitCreation() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create a default end date if none provided (30 days from now)
      const habitEndDate = endDate || new Date(new Date().setDate(new Date().getDate() + 30))
      ...
      const newHabit = await addHabit({
        ...
        xpReward: XP_VALUES.HABIT_CREATION
      })
      ...
      toast({
        ...
        description: `You earned ${XP_VALUES.HABIT_CREATION} XP for creating a new habit. It's now in your daily tasks.`,
      ...
      // Redirect to habits page after short delay
      setTimeout(() => {
        router.push('/habits')
      }, 1500)
          ...
          <CardContent>
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              >
                <Button 
                  type="submit" 
                  className="w-full bg-gradient-to-r from-[#4cc9f0] to-[#4361ee] hover:from-[#4361ee] hover:to-[#4cc9f0] text-white font-semibold py-3 rounded-lg shadow-lg transform transition-all hover:scale-[1.02] disabled:opacity-70"
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Rocket className="mr-2 h-5 w-5" /> Launch Habit Mission
                    </>
                  )}
                </Button>
              </motion.div>
